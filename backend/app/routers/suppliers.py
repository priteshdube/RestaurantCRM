...
async def get_supplier(supplier_id: str, user=Depends(get_current_user)):
    result = supabase.table("suppliers").select(
        "*, inventory_items(id, name, quantity, unit, status)"
    ).eq("id", supplier_id).eq("user_id", user["user_id"]).single().execute()
    if not result.data:
        raise HTTPException(status_code=404, detail="Supplier not found")
    return result.data
...
async def update_supplier(
    ...
    data = {k: v for k, v in supplier.model_dump().items() if v is not None}
    if not data:
        raise HTTPException(status_code=400, detail="No fields to update")
    result = supabase.table("suppliers").update(data).eq(
        "id", supplier_id
    ).eq("user_id", user["user_id"]).execute()
    return result.data[0]
...
async def delete_supplier(supplier_id: str, user=Depends(get_current_user)):
    supabase.table("suppliers").update(
        {"is_active": False}
    ).eq("id", supplier_id).eq("user_id", user["user_id"]).execute()
